...
class TestGlovesResponse(TestCase):
    ...
    def test_get_logged_user_return_404_when_object_not_exist(self):
        url = reverse("gloves-detail", kwargs={"pk": 100000})
    ...
    def test_post_logged_have_permissions_user_return_405(self):
        url = reverse("gloves-detail", kwargs={"pk": 1})

        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="add_gloves")
    ...
    def test_patch_logged_have_permissions_user_return_right_value(self):
        object_to_patch = Gloves.objects.filter(name="Gloves").first()
        url = reverse("gloves-detail", kwargs={"pk": object_to_patch.id})

        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="change_gloves")
        self.user.user_permissions.add(self.permission)
    ...
    def test_delete_logged_have_permissions_user_return_204(self):
        object_to_delete = Gloves.objects.filter(name="Gloves").first()
        url = reverse("gloves-detail", kwargs={"pk": object_to_delete.id})

        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="delete_gloves")
        self.user.user_permissions.add(self.permission)
