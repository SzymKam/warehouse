...
class TestDrugsResponse(TestCase):
    def setUp(self) -> None:
        ...
        self.drug_1 = Drug.objects.create(
            name="ASA", active_substance="Acidum acetylsalicylicum"
        )
        self.drug_2 = Drug.objects.create(
    ...
    def test_get_logged_user_return_right_values_with_two_objects(self):
        ...
        self.assertEqual(response.data[0]["name"], "ASA")
        self.assertEqual(
            response.data[0]["active_substance"], "Acidum acetylsalicylicum"
        )

        self.assertEqual(response.data[1]["name"], "Adrenalin")
        self.assertEqual(response.data[1]["active_substance"], "Epinephrinum")
    ...
    def test_post_logged_have_permissions_user_return_405(self):
        url = reverse("drug-list")
        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="add_drug")
    ...
    def test_patch_logged_have_permissions_user_return_405(self):
        ...
        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="change_drug")
        self.user.user_permissions.add(self.permission)
    ...
    def test_delete_logged_have_permissions_user_return_405(self):
        ...
        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="delete_drug")
        self.user.user_permissions.add(self.permission)
