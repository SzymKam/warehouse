...
from containers.models import OropharyngealTube, Container
...
class TestOPATubeResponse(TestCase):
    def setUp(self) -> None:
        ...
        self.element_1 = OropharyngealTube.objects.create(name="OPA tube", size="1")
        self.element_2 = OropharyngealTube.objects.create(name="OPA tube", size="3")

    def test_get_not_logged_user_return_403(self):
        object_to_get = OropharyngealTube.objects.filter(name="OPA tube").first()
        url = reverse("oropharyngealtube-detail", kwargs={"pk": object_to_get.id})
    ...
    def test_get_logged_user_return_404_when_object_not_exist(self):
        url = reverse("oropharyngealtube-detail", kwargs={"pk": 100000})
    ...
    def test_post_logged_have_permissions_user_return_405(self):
        url = reverse("oropharyngealtube-detail", kwargs={"pk": 1})

        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="add_oropharyngealtube")
    ...
    def test_patch_logged_have_permissions_user_return_right_value(self):
        object_to_patch = OropharyngealTube.objects.filter(name="OPA tube").first()
        url = reverse("oropharyngealtube-detail", kwargs={"pk": object_to_patch.id})

        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="change_oropharyngealtube")
        self.user.user_permissions.add(self.permission)
    ...
    def test_delete_logged_have_permissions_user_return_204(self):
        object_to_delete = OropharyngealTube.objects.filter(name="OPA tube").first()
        url = reverse("oropharyngealtube-detail", kwargs={"pk": object_to_delete.id})

        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="delete_oropharyngealtube")
        self.user.user_permissions.add(self.permission)
