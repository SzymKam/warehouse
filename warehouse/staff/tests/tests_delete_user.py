...
class StaffDeleteTest(TestCase):
    def setUp(self) -> None:
        ...
        self.user_1 = StaffModel.objects.create(
            username="nimda", password=secrets.token_bytes(nbytes=8)
        )
        self.user_2 = StaffModel.objects.create(
            username="test_2", password=secrets.token_bytes(nbytes=8)
        )
        self.user_3 = StaffModel.objects.create(
            username="test_3", password=secrets.token_bytes(nbytes=8)
        )
