...
from containers.models import Needle, Container
from staff.models import StaffModel
from django.urls import reverse
from rest_framework import status


class TestNeedleResponse(TestCase):
    def setUp(self) -> None:
        ...
        self.element_1 = Needle.objects.create(name="Needle", size="Blue - 0.6mm")
        self.element_2 = Needle.objects.create(name="Needle", size="Pink - 1.2mm")

    def test_get_not_logged_user_return_403(self):
        object_to_get = Needle.objects.filter(name="Needle").first()
        url = reverse("needle-detail", kwargs={"pk": object_to_get.id})
    ...
    def test_get_logged_user_return_404_when_object_not_exist(self):
        url = reverse("needle-detail", kwargs={"pk": 100000})
    ...
    def test_post_logged_have_permissions_user_return_405(self):
        url = reverse("needle-detail", kwargs={"pk": 1})

        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="add_needle")
    ...
    def test_patch_logged_have_permissions_user_return_right_value(self):
        object_to_patch = Needle.objects.filter(name="Needle").first()
        url = reverse("needle-detail", kwargs={"pk": object_to_patch.id})

        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="change_needle")
        self.user.user_permissions.add(self.permission)
    ...
    def test_delete_logged_have_permissions_user_return_204(self):
        object_to_delete = Needle.objects.filter(name="Needle").first()
        url = reverse("needle-detail", kwargs={"pk": object_to_delete.id})

        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="delete_needle")
        self.user.user_permissions.add(self.permission)
