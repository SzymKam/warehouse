...
class TestFluidResponse(TestCase):
    ...
    def test_get_not_logged_user_return_403(self):
        object_to_get = Fluid.objects.filter(name="Optilyte").first()
        url = reverse("fluid-detail", kwargs={"pk": object_to_get.id})
    ...
    def test_get_logged_user_return_404_when_object_not_exist(self):
        url = reverse("fluid-detail", kwargs={"pk": 100000})
    ...
    def test_post_logged_have_permissions_user_return_405(self):
        url = reverse("fluid-detail", kwargs={"pk": 1})

        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="add_fluid")
        self.user.user_permissions.add(self.permission)

        data = {"name": "Plasmalyte", "volume": "500ml"}
    ...
    def test_patch_logged_have_permissions_user_return_right_value(self):
        object_to_patch = Fluid.objects.filter(name="Optilyte").first()
        url = reverse("fluid-detail", kwargs={"pk": object_to_patch.id})
    ...
    def test_delete_logged_have_permissions_user_return_204(self):
        object_to_delete = Fluid.objects.filter(name="Optilyte").first()
        url = reverse("fluid-detail", kwargs={"pk": object_to_delete.id})
