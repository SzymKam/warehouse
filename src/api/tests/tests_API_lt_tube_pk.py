...
class TestLtTubeResponse(TestCase):
    def setUp(self) -> None:
    ...
    def test_get_not_logged_user_return_403(self):
        object_to_get = LtTube.objects.filter(name="LT tube").first()
        url = reverse("lttube-detail", kwargs={"pk": object_to_get.id})
    ...
    def test_get_logged_user_return_404_when_object_not_exist(self):
        url = reverse("lttube-detail", kwargs={"pk": 100000})
    ...
    def test_post_logged_have_permissions_user_return_405(self):
        url = reverse("lttube-detail", kwargs={"pk": 1})

        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="add_lttube")
    ...
    def test_patch_logged_have_permissions_user_return_right_value(self):
        object_to_patch = LtTube.objects.filter(name="LT tube").first()
        url = reverse("lttube-detail", kwargs={"pk": object_to_patch.id})

        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="change_lttube")
        self.user.user_permissions.add(self.permission)
    ...
    def test_delete_logged_have_permissions_user_return_204(self):
        object_to_delete = LtTube.objects.filter(name="LT tube").first()
        url = reverse("lttube-detail", kwargs={"pk": object_to_delete.id})

        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="delete_lttube")
        self.user.user_permissions.add(self.permission)
