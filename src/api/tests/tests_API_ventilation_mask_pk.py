...
class TestVentilationMaskResponse(TestCase):
    ...
    def test_get_not_logged_user_return_403(self):
        object_to_get = VentilationMask.objects.filter(name="Ventilation mask").first()
        url = reverse("ventilationmask-detail", kwargs={"pk": object_to_get.id})
    ...
    def test_get_logged_user_return_404_when_object_not_exist(self):
        url = reverse("ventilationmask-detail", kwargs={"pk": 100000})
    ...
    def test_post_logged_have_permissions_user_return_405(self):
        url = reverse("ventilationmask-detail", kwargs={"pk": 1})
    ...
    def test_patch_logged_have_permissions_user_return_right_value(self):
        object_to_patch = VentilationMask.objects.filter(
            name="Ventilation mask"
        ).first()
        url = reverse("ventilationmask-detail", kwargs={"pk": object_to_patch.id})
    ...
    def test_delete_logged_have_permissions_user_return_204(self):
        object_to_delete = VentilationMask.objects.filter(
            name="Ventilation mask"
        ).first()
        url = reverse("ventilationmask-detail", kwargs={"pk": object_to_delete.id})
