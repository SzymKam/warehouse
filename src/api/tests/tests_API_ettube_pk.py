...
from containers.models import EndotrachealTube, Container
from staff.models import StaffModel
from django.urls import reverse
from rest_framework import status


class TestEtTubeResponse(TestCase):
    def setUp(self) -> None:
        ...
        self.element_1 = EndotrachealTube.objects.create(name="ET tube", size="2.5")
        self.element_2 = EndotrachealTube.objects.create(name="ET tube", size="10.0")

    def test_get_not_logged_user_return_403(self):
        object_to_get = EndotrachealTube.objects.filter(name="ET tube").first()
        url = reverse("endotrachealtube-detail", kwargs={"pk": object_to_get.id})
    ...
    def test_get_logged_user_return_404_when_object_not_exist(self):
        url = reverse("endotrachealtube-detail", kwargs={"pk": 100000})
    ...
    def test_post_logged_have_permissions_user_return_405(self):
        url = reverse("endotrachealtube-detail", kwargs={"pk": 1})

        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="add_endotrachealtube")
    ...
    def test_patch_logged_have_permissions_user_return_right_value(self):
        object_to_patch = EndotrachealTube.objects.filter(name="ET tube").first()
        url = reverse("endotrachealtube-detail", kwargs={"pk": object_to_patch.id})

        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="change_endotrachealtube")
        self.user.user_permissions.add(self.permission)
    ...
    def test_delete_logged_have_permissions_user_return_204(self):
        object_to_delete = EndotrachealTube.objects.filter(name="ET tube").first()
        url = reverse("endotrachealtube-detail", kwargs={"pk": object_to_delete.id})

        self.client.force_login(self.user)
        self.permission = Permission.objects.get(codename="delete_endotrachealtube")
        self.user.user_permissions.add(self.permission)
